...
Elevates verl's v1 synchronous PPO trainer from a single agent to a **fleet of agents with rigid,
...
- **Agent**: one role in the fleet (`verl/experimental/multiagent/config/agent_config.py:AgentConfig`),
  identified by `agent_id`, with its own `system_prompt` and a `backend`:
  - `trainable_verl`: a verl-managed rollout server whose weights PPO updates. Exactly one agent
    must have `model_ref: main` (reuses the trainer's primary `actor_rollout_ref` config and
    worker group); every other trainable agent is configured via its own `actor_rollout_ref`
    sub-config plus `n_gpus_per_node`/`nnodes`, and agents that set the *same* `model_ref` are the
  ...
  - `external_api`: an external OpenAI/Anthropic-compatible endpoint (never trained) -- for a
    ...
    (`AgentConfig.external_api`) keep a slow/rate-limited provider from stalling the rest of
    rollout (see `agent_backend.py:ExternalAPIAgentBackend`).
- **Communication policy** (`policy.py`): decides who speaks next each turn. v1 ships
  ...
  optional early termination when a configured agent's turn reports
...
- **Episode**: one multi-turn conversation, driven by `agent_loop.py:MultiAgentLoop`
  ...
  `AgentLoopOutput` per **trainable** agent -- non-trainable (frozen/external) agents' text still
  shapes what trainable agents are conditioned on, but they get no trainable row of their own.
- **Reward**: v1 default is a single shared episodic reward, broadcast to every trainable agent's
  ...
  the agent who took the episode's last turn is last in the list.

## Configuring a new fleet

Add a file under `config/agent_fleet/<name>.yaml` (see `math_teacher_student.yaml` for a worked
example), then point `config/multiagent_ppo_trainer.yaml`'s
`defaults: - agent_fleet@multiagent: <name>` at it (or override on the CLI:
`+agent_fleet@multiagent=<name>`). Each agent needs:
...
agents:
  main:                        # agent_id
    ...
    model_ref: main             # exactly one agent must be model_ref: main

  weak_student:                 # a second trainable agent on its own model/pool
    agent_id: weak_student
    system_prompt: "..."
    backend: trainable_verl
    model_ref: weak_student
    actor_rollout_ref: {...}    # full actor_rollout_ref-shaped sub-config for this agent's model
    n_gpus_per_node: 2
    nnodes: 1
...
Two agent_ids can share one model by giving them the same `model_ref` (e.g. two "critic" roles
...
- **Resource pools**: `trainer.py:MultiAgentPPOTrainer._init_resource_pool_mgr` gives each
  *unique* `model_ref` among non-main trainable agents its own dedicated Ray resource pool
  (multiple agent_ids sharing a `model_ref` collapse onto one pool/worker group/`LLMServerManager`
  in `_setup_agent_groups`), generalizing the existing `teacher_pool` precedent for on-policy
  distillation. Frozen agents still get one dedicated pool per agent_id via `FrozenAgentManager`.
  Agent ids/model_refs are used directly as string keys into `ResourcePoolManager`'s mapping --
  `Role` (`verl/trainer/ppo/utils.py`) can't be subclassed (Python disallows extending an `Enum`
  that already has members).
- **Per-group training**: the six single-agent pipeline methods (`_balance_batch`,
  ...
  every agent_id sharing that model_ref into one on-policy training batch), temporarily rebind
  `self.actor_rollout_wg`/`self.config`/`self.tokenizer` to the group
  (`trainer.py:_bind_agent_group`), call the inherited method, restore. This must stay a
  sequential loop across groups -- see the docstring on `_bind_agent_group` for why.
- **Token-level correctness**: `agent_loop.py:AgentTurnRenderer` incrementally builds each
  trainable agent's own view of the episode (own turns masked in for training, everyone else's
  ...
  `tests/experimental/multiagent/test_agent_turn_renderer_on_cpu.py`.
- **Frozen verl-hosted agents** reuse `verl/experimental/teacher_loop/teacher_model.py`'s
  ...
  agent must generate full responses, while the distillation path's
  `validate_and_prepare_for_distillation` collapses response length to 1 for a single-token
  logprob forward pass -- semantics that don't apply here (see `frozen_agent_manager.py`).
...
- **Per-agent reward** -- v1 is shared-episodic-reward-only, by construction of
  `_agent_loop_postprocess`'s broadcast. The `agent_id`/`model_ref` tagging already in place on
  every trainable turn is what a future per-agent reward function would key off.
- **Prefix/KV-cache sharing** across agents that share a model.
...
- **Mid-episode correctness checks** -- `RigidSequencePolicy`'s early-termination hook
  (`termination_agent_id`/`termination_metric`) exists, but no shipped `AgentBackend` sets
  `AgentTurnResult.metrics["done"]`; only the post-episode reward function scores correctness.
