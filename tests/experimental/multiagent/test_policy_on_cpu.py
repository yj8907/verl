...
def _drive(policy, max_iters=10, done_after_agent=None, done_after_n_turns=None):
    """Run a policy to completion, marking done=True once `done_after_agent` has spoken
    `done_after_n_turns` times, and return the sequence of agent ids that spoke."""
    state = EpisodeState()
    turns = []
    counts = {}
    for _ in range(max_iters):
        next_agent = policy.next_agent(state)
        if next_agent is None:
            break
        turns.append(next_agent)
        counts[next_agent] = counts.get(next_agent, 0) + 1
        done = next_agent == done_after_agent and counts[next_agent] >= (done_after_n_turns or 0)
        state = EpisodeState(
            turn_index=state.turn_index + 1,
            last_agent_id=next_agent,
            last_result=AgentTurnResult(text="x", trainable=True, metrics={"done": done}),
        )
    return turns
...
class TestRigidSequencePolicy(unittest.TestCase):
    ...
    def test_single_agent_turn_order_repeats(self):
        policy = RigidSequencePolicy(CommunicationPolicyConfig(turn_order=["main"], max_turns=3))
        assert _drive(policy) == ["main", "main", "main"]

    def test_terminates_early_on_configured_agent_done_signal(self):
        policy = RigidSequencePolicy(
            CommunicationPolicyConfig(
                turn_order=["main", "oracle"], max_turns=10, termination_agent_id="main", termination_metric="done"
            )
        )
        turns = _drive(policy, done_after_agent="main", done_after_n_turns=2)
        assert turns == ["main", "oracle", "main"]

    def test_termination_only_checked_on_the_configured_agent(self):
        # oracle reports done, but termination_agent_id is "main" -- must not stop early.
        policy = RigidSequencePolicy(
            CommunicationPolicyConfig(
                turn_order=["main", "oracle"], max_turns=4, termination_agent_id="main", termination_metric="done"
            )
        )
        turns = _drive(policy, done_after_agent="oracle", done_after_n_turns=1)
        assert turns == ["main", "oracle", "main", "oracle"]

    def test_no_termination_agent_configured_runs_to_max_turns(self):
        policy = RigidSequencePolicy(CommunicationPolicyConfig(turn_order=["a"], max_turns=2))
        assert _drive(policy, done_after_agent="a", done_after_n_turns=1) == ["a", "a"]
